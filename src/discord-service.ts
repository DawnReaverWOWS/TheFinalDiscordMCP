// Configure FFmpeg path for @discordjs/voice BEFORE importing voice
...
import { getGuildVoice } from './voice-settings.js';
...
export class DiscordService {
  ...
  async initialize(): Promise<void> {
    ...
    // SECURITY: Validate token format (Bot tokens have specific structure)
    // Format: Base64(bot_id).Base64(timestamp).HMAC
    const tokenParts = token.split('.');
    if (tokenParts.length !== 3) {
      console.error("ERROR: DISCORD_TOKEN appears malformed. Expected format: XXX.XXX.XXX");
      process.exit(1);
    }

    // Warn if token might be exposed in logs
    if (token.length < 50) {
      console.error("WARNING: DISCORD_TOKEN seems unusually short. Verify it's correct.");
    }

    // SECURITY: Validate that required intents are configured
    // Required intents for this bot's functionality
    ...
    // Get configured intents from client options
    const configuredIntents = this.client.options.intents.toArray();
    const intentBitValues = configuredIntents.map(intent => {
      // GatewayIntentBits values are already numeric
      return typeof intent === 'number' ? intent : 0;
    });

    const intentValidation = SecurityUtils.validateIntents(intentBitValues, requiredIntents);
    if (!intentValidation.valid) {
      console.error(`WARNING: Missing required intents: ${intentValidation.missing.join(', ')}`);
      console.error("Some bot features may not work correctly without these intents.");
    }
  ...
  /**
   * Send startup notification to configured channel (or DM to owner as fallback)
   */
  private async sendStartupNotification(): Promise<void> {
    const notifyChannelId = process.env.BOT_NOTIFY_CHANNEL_ID;
    const ownerId = process.env.BOT_OWNER_ID;

    if (!notifyChannelId && !ownerId) {
      return; // No notification target configured, skip
    }
    ...
    try {
      ...
      if (notifyChannelId) {
        ...
        const fetchedChannel = await this.client.channels.fetch(notifyChannelId);
        if (!fetchedChannel || !fetchedChannel.isTextBased()) {
          console.error(`Notify channel ${notifyChannelId} not found or not a text channel`);
          return;
        }
        channel = fetchedChannel as TextChannel;
        targetDescription = `channel #${(channel as TextChannel).name}`;
      } else {
        // Fallback to DM
        const owner = await this.client.users.fetch(ownerId!);
        channel = await owner.createDM();
        targetDescription = `owner ${owner.tag}`;
      }
      ...
      const hostname = process.env.VM_NAME || osModule.hostname();
      const platform = osModule.platform();
      const arch = osModule.arch();
      const totalMem = Math.round(osModule.totalmem() / 1024 / 1024 / 1024 * 10) / 10;
      const freeMem = Math.round(osModule.freemem() / 1024 / 1024 / 1024 * 10) / 10;
      ...
      const embed = new EmbedBuilder()
        .setColor(0x00FF00)
        .setTitle(`Bot Online - v${BOT_VERSION}`)
        .setDescription(`**${this.client.user?.tag}** is now online!`)
        .addFields(
          { name: '🖥️ Instance', value: hostname, inline: true },
          { name: '☁️ Platform', value: `${platform}/${arch}`, inline: true },
          { name: '📦 Node.js', value: nodeVersion, inline: true },
          { name: '💾 Memory', value: `${freeMem}/${totalMem} GB free`, inline: true },
          { name: '🔢 CPUs', value: `${cpus} cores`, inline: true },
          { name: '⏱️ System Uptime', value: uptimeStr, inline: true },
          { name: '🕐 Timestamp', value: timestamp, inline: false }
        )
        .setFooter({ text: 'Oracle Cloud Infrastructure • GitHub Actions Deploy' })
        .setTimestamp();

      await channel.send({ embeds: [embed] });
      console.error(`Startup notification sent to ${targetDescription}`);
    } catch (error) {
 ...
 - Tier: ${guild.premiumTier}`;
  ...
  async joinVoiceChannel(guildId: string, channelId: string): Promise<string> {
    ...
    try {
      ...
      // SECURITY FIX: Add proper cleanup on disconnect events
      connection.on(VoiceConnectionStatus.Disconnected, () => {
        // Clean up on unexpected disconnect
        this.voiceConnections.delete(guild.id);
        const player = this.audioPlayers.get(guild.id);
        if (player) {
          player.stop();
          this.audioPlayers.delete(guild.id);
        }
        console.error(`Voice connection disconnected for guild ${guild.id}`);
      });

      connection.on(VoiceConnectionStatus.Destroyed, () => {
        // Ensure cleanup when connection is destroyed
        this.voiceConnections.delete(guild.id);
        const player = this.audioPlayers.get(guild.id);
        if (player) {
          player.stop();
  ...
  async speakText(guildId: string, text: string, voiceOverride?: string): Promise<string> {
    this.ensureReady();
    const resolvedGuildId = this.resolveGuildId(guildId);

    const connection = this.voiceConnections.get(resolvedGuildId);
    if (!connection) {
      throw new Error("Bot is not in a voice channel. Use !join <channel name> first.");
    }
    ...
    try {
      // Use override voice, or get guild's selected voice
      const voice = voiceOverride || getGuildVoice(resolvedGuildId);

      // Generate TTS audio to temp file
      const tts = new MsEdgeTTS();
      await tts.setMetadata(voice, OUTPUT_FORMAT.AUDIO_24KHZ_48KBITRATE_MONO_MP3);

      // toFile takes a DIRECTORY path, returns the actual file path created
  ...
  async timeoutMember(guildId: string | undefined, userId: string, duration: number, reason?: string): Promise<string> {
    ...
    try {
      ...
      // Duration parameter is now in SECONDS (not minutes) for consistency
      const timeoutDuration = duration * 1000; // Convert seconds to milliseconds
      ...
      return `Successfully timed out ${member.user.username} (ID: ${userId}) for ${displayDuration}. Reason: ${reason || 'No reason provided'}`;
    } catch (error) {
    ...
    try {
      ...
      for (const message of imageMessages) {
        ...
        for (const attachment of imageAttachments) {
          ...
          if (downloadImages) {
            try {
              // SECURITY: Build URL from scratch using only validated attachment properties
              // This completely breaks any taint chain from user input.
              // Discord attachment objects have: id (snowflake), name (filename), url
              // We reconstruct using hardcoded domain + validated IDs only.
              const attachmentId = attachment.id;
              const filename = attachment.name;

              // Validate attachment ID is a valid snowflake (17-20 digits)
              if (!/^\d{17,20}$/.test(attachmentId)) {
                imageInfo.downloadError = 'Security: Invalid attachment ID format';
              } else if (!/^[a-zA-Z0-9_\-.\s]+$/.test(filename)) {
                // Validate filename contains only safe characters
                imageInfo.downloadError = 'Security: Invalid filename format';
              } else {
                // Construct URL using ONLY hardcoded domain and validated IDs
                // channelId comes from Discord API (message.channel.id), not user input
                const safeChannelId = message.channel.id;
                const DISCORD_CDN = 'https://cdn.discordapp.com'; // Hardcoded safe domain
                const safeUrl = `${DISCORD_CDN}/attachments/${safeChannelId}/${attachmentId}/${encodeURIComponent(filename)}`;
