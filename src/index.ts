...
// Check if running in HTTP-only mode (no Discord bot connection)
const HTTP_ONLY_MODE = process.env.HTTP_ONLY === 'true';

// Initialize Discord service
async function initializeDiscord() {
  if (HTTP_ONLY_MODE) {
    console.log('Running in HTTP-only mode - Discord bot connection disabled');
    console.log('This instance will serve API requests but cannot interact with Discord directly');
    return;
  }
  ...
  try {
    ...
    switch (name) {
      ...
      case 'read_images': {
        ...
        // snyk:ignore javascript/Ssrf - channelId/messageId are Discord snowflake IDs, not URLs.
        // URLs are fetched from Discord API and validated against Discord CDN whitelist in readImages().
        const result = await discordService.readImages(
          parsed.channelId,
          parsed.messageId,
          parsed.limit,
...
async function main() {
  try {
    ...
    if (useHttp) {
      ...
      // CORS configuration for VM1 (frontend) access
      const allowedOrigins = [
        'http://10.0.0.174:3000',     // VM1 private IP (Next.js)
        'http://10.0.0.174',           // VM1 private IP
        'http://129.153.70.66',        // VM1 public IP
        'http://129.153.70.66:3000',   // VM1 public IP with port
      ...
      const httpServer = createServer(async (req, res) => {
        ...
        // CORS headers - check if origin is allowed
        const isAllowedOrigin = allowedOrigins.includes(origin) ||
          origin.startsWith('http://10.0.') ||  // Allow all VCN private IPs
          process.env.CORS_ALLOW_ALL === 'true'; // Dev mode: allow all

        // SECURITY: Never use wildcard '*' for CORS origin to prevent CSRF attacks
        if (isAllowedOrigin && origin) {
          // Only reflect the actual origin, never use wildcard
          res.setHeader('Access-Control-Allow-Origin', origin);
        } else if (process.env.NODE_ENV !== 'production' && origin) {
          // In development, reflect origin but still require it to be present
          res.setHeader('Access-Control-Allow-Origin', origin);
        }
        // If no origin header present (e.g., same-origin requests), don't set CORS header
        ...
        try {
          ...
          // ============================================================
          ...
          } else if (url.pathname === '/api/server' && req.method === 'GET') {
            // GET /api/server - Get server info
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getServerInfo(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
            ...
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.listChannels(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
            ...
            try {
              ...
              const result = await discordService.getMembers(guildId, limit);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/roles' && req.method === 'GET') {
            // GET /api/roles - List roles
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getRoles(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
            ...
            // GET /api/messages?channelId=xxx&count=50 - Read messages from channel
            try {
              ...
              const result = await discordService.readMessages(channelId, count);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/messages' && req.method === 'POST') {
            ...
            req.on('end', async () => {
              try {
                ...
                if (!channelId || !message) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId and message are required' }));
                  return;
                }
            ...
            // GET /api/events - List server events
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getEvents(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/emojis' && req.method === 'GET') {
            // GET /api/emojis - List emojis
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getEmojis(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/stickers' && req.method === 'GET') {
            // GET /api/stickers - List stickers
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getStickers(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/invites' && req.method === 'GET') {
            // GET /api/invites - List invites
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getInvites(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
            ...
            // GET /api/webhooks?channelId=xxx - List webhooks
            try {
              const channelId = url.searchParams.get('channelId');
              if (!channelId) {
                res.writeHead(400, { 'Content-Type': 'application/json' });
              ...
              const result = await discordService.listWebhooks(channelId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/automod' && req.method === 'GET') {
            // GET /api/automod - List automod rules
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getAutomodRules(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
            ...
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getServerStats(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/tools' && req.method === 'GET') {
            // GET /api/tools - List all available MCP tools
            try {
              const tools = getAllTools();
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: tools, count: tools.length }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: false, error: (error as Error).message }));
            }
          ...
          // ============================================================
          ...
          } else if (url.pathname === '/api/bans' && req.method === 'GET') {
            // GET /api/bans - List server bans
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getBans(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/auditlog' && req.method === 'GET') {
            // GET /api/auditlog - Get audit log entries
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const limit = parseInt(url.searchParams.get('limit') || '50');
              const result = await discordService.getAuditLogs(guildId, limit);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/threads' && req.method === 'GET') {
            // GET /api/threads - List active threads
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getActiveThreads(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/pinned' && req.method === 'GET') {
            // GET /api/pinned?channelId=xxx - Get pinned messages
            try {
              const channelId = url.searchParams.get('channelId');
              if (!channelId) {
                res.writeHead(400, { 'Content-Type': 'application/json' });
              ...
              const result = await discordService.getPinnedMessages(channelId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/voice' && req.method === 'GET') {
            // GET /api/voice - Get active voice connections
            try {
              const result = await discordService.getVoiceConnections();
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
          ...
          } else if (url.pathname === '/api/structure' && req.method === 'GET') {
            // GET /api/structure - Get channel structure
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getChannelStructure(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/widget' && req.method === 'GET') {
            // GET /api/widget - Get server widget
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getServerWidget(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/welcome' && req.method === 'GET') {
            // GET /api/welcome - Get welcome screen
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              const result = await discordService.getWelcomeScreen(guildId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/member' && req.method === 'GET') {
            // GET /api/member?userId=xxx - Get member info
            try {
              const guildId = url.searchParams.get('guildId') || undefined;
              ...
              const result = await discordService.getMemberInfo(guildId, userId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
            ...
            // GET /api/permissions?channelId=xxx - Get channel permissions
            try {
              const channelId = url.searchParams.get('channelId');
              if (!channelId) {
                res.writeHead(400, { 'Content-Type': 'application/json' });
              ...
              const result = await discordService.getChannelPermissions(channelId);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
            ...
            // GET /api/export?channelId=xxx&format=json - Export chat log
            try {
              const channelId = url.searchParams.get('channelId');
              ...
              const result = await discordService.exportChatLog(channelId, format, limit);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/ban' && req.method === 'POST') {
            // POST /api/ban - Ban a member
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, userId, reason } = JSON.parse(body);
                if (!userId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'userId is required' }));
                  return;
                }
                const result = await discordService.banMember(guildId, userId, reason);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/unban' && req.method === 'POST') {
            // POST /api/unban - Unban a member
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.unbanMember(guildId, userId, reason);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/kick' && req.method === 'POST') {
            // POST /api/kick - Kick a member
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.kickMember(guildId, userId, reason);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/timeout' && req.method === 'POST') {
            // POST /api/timeout - Timeout a member
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, userId, duration, reason } = JSON.parse(body);
                if (!userId || !duration) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'userId and duration are required' }));
                  return;
                }
                const result = await discordService.timeoutMember(guildId, userId, duration, reason);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/roles/add' && req.method === 'POST') {
            // POST /api/roles/add - Add role to member
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, userId, roleId } = JSON.parse(body);
                if (!userId || !roleId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'userId and roleId are required' }));
                  return;
                }
                const result = await discordService.addRoleToMember(guildId, userId, roleId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/roles/remove' && req.method === 'POST') {
            // POST /api/roles/remove - Remove role from member
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.removeRoleFromMember(guildId, userId, roleId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/roles/create' && req.method === 'POST') {
            // POST /api/roles/create - Create a new role
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, name, color, permissions } = JSON.parse(body);
                if (!name) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'name is required' }));
                  return;
                }
                const result = await discordService.createRole(guildId, name, color, permissions);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // ============================================================
          // CHANNEL MANAGEMENT (POST)
          // ============================================================

          } else if (url.pathname === '/api/channels/create' && req.method === 'POST') {
            // POST /api/channels/create - Create a channel
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, name, type, categoryId } = JSON.parse(body);
                if (!name) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'name is required' }));
                  return;
                ...
                switch (type) {
                  case 'voice':
                    result = await discordService.createVoiceChannel(guildId, name, categoryId);
                    break;
                  case 'forum':
                    result = await discordService.createForumChannel(guildId, name, categoryId);
                    break;
                  case 'announcement':
                    result = await discordService.createAnnouncementChannel(guildId, name, categoryId);
                    break;
                  case 'stage':
                    result = await discordService.createStageChannel(guildId, name, categoryId);
                    break;
                  case 'category':
                    result = await discordService.createCategory(guildId, name);
                    break;
                  default:
                    result = await discordService.createTextChannel(guildId, name, categoryId);
                }
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
          ...
          } else if (url.pathname === '/api/channels/delete' && req.method === 'POST') {
            // POST /api/channels/delete - Delete a channel
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, channelId } = JSON.parse(body);
                if (!channelId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId is required' }));
                  return;
                }
                const result = await discordService.deleteChannel(guildId, channelId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // ============================================================
          // EVENT MANAGEMENT (POST)
          // ============================================================

          } else if (url.pathname === '/api/events/create' && req.method === 'POST') {
            // POST /api/events/create - Create an event
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, name, description, startTime, endTime, location, channelId } = JSON.parse(body);
                if (!name || !startTime) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'name and startTime are required' }));
                  return;
                }
                const result = await discordService.createEvent(guildId, name, description, startTime, endTime, location, channelId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/events/delete' && req.method === 'POST') {
            // POST /api/events/delete - Delete an event
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, eventId } = JSON.parse(body);
                if (!eventId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'eventId is required' }));
                  return;
                }
                const result = await discordService.deleteEvent(guildId, eventId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // ============================================================
          // INVITE MANAGEMENT (POST)
          // ============================================================

          } else if (url.pathname === '/api/invites/create' && req.method === 'POST') {
            // POST /api/invites/create - Create an invite
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, maxAge, maxUses, temporary } = JSON.parse(body);
                if (!channelId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId is required' }));
                  return;
                }
                const result = await discordService.createInvite(channelId, maxAge, maxUses, temporary);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/invites/delete' && req.method === 'POST') {
            // POST /api/invites/delete - Delete an invite
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { inviteCode } = JSON.parse(body);
                if (!inviteCode) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'inviteCode is required' }));
                  return;
                }
                const result = await discordService.deleteInvite(inviteCode);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // MESSAGES - Full CRUD
          // ============================================
          } else if (url.pathname === '/api/messages/edit' && req.method === 'PUT') {
            // PUT /api/messages/edit - Edit a message
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, messageId, content } = JSON.parse(body);
                if (!channelId || !messageId || !content) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId, messageId, and content are required' }));
                  return;
                }
                const result = await discordService.editMessage(channelId, messageId, content);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/messages/delete' && req.method === 'DELETE') {
            ...
            req.on('end', async () => {
              try {
                ...
                if (!channelId || !messageId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId and messageId are required' }));
                  return;
                }
          ...
          } else if (url.pathname === '/api/messages/bulk-delete' && req.method === 'POST') {
            // POST /api/messages/bulk-delete - Bulk delete messages
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, count } = JSON.parse(body);
                if (!channelId || !count) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId and count are required' }));
                  return;
                }
                const result = await discordService.bulkDeleteMessages(channelId, count);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/messages/pin' && req.method === 'POST') {
            ...
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.pinMessage(channelId, messageId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/messages/unpin' && req.method === 'POST') {
            // POST /api/messages/unpin - Unpin a message
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.unpinMessage(channelId, messageId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/messages/react' && req.method === 'POST') {
            // POST /api/messages/react - Add reaction to message
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, messageId, emoji } = JSON.parse(body);
                if (!channelId || !messageId || !emoji) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId, messageId, and emoji are required' }));
                  return;
                }
                const result = await discordService.addReaction(channelId, messageId, emoji);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/messages/unreact' && req.method === 'POST') {
            // POST /api/messages/unreact - Remove reaction from message
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.removeReaction(channelId, messageId, emoji);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // THREADS - Full CRUD
          // ============================================
          } else if (url.pathname === '/api/threads/create' && req.method === 'POST') {
            // POST /api/threads/create - Create a thread
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, name, autoArchiveDuration, messageId } = JSON.parse(body);
                if (!channelId || !name) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId and name are required' }));
                  return;
                }
                const result = await discordService.createThread(channelId, name, autoArchiveDuration, messageId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/threads/archive' && req.method === 'POST') {
            // POST /api/threads/archive - Archive a thread
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.archiveThread(threadId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/threads/unarchive' && req.method === 'POST') {
            // POST /api/threads/unarchive - Unarchive a thread
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.unarchiveThread(threadId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/threads/lock' && req.method === 'POST') {
            // POST /api/threads/lock - Lock a thread
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.lockThread(threadId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/threads/unlock' && req.method === 'POST') {
            // POST /api/threads/unlock - Unlock a thread
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                const result = await discordService.unlockThread(threadId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // WEBHOOKS - Full CRUD
          // ============================================
          } else if (url.pathname === '/api/webhooks/create' && req.method === 'POST') {
            // POST /api/webhooks/create - Create a webhook
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, name, avatar } = JSON.parse(body);
                if (!channelId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId is required' }));
                  return;
                }
                const result = await discordService.createWebhook(channelId, name || 'Webhook');
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/webhooks/delete' && req.method === 'DELETE') {
            ...
            req.on('end', async () => {
              try {
                ...
                if (!webhookId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'webhookId is required' }));
                  return;
                }
                const result = await discordService.deleteWebhook(webhookId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/webhooks/send' && req.method === 'POST') {
            // POST /api/webhooks/send - Send message via webhook
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { webhookUrl, content, username, avatarUrl } = JSON.parse(body);
                if (!webhookUrl || !content) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'webhookUrl and content are required' }));
                  return;
                }
                const result = await discordService.sendWebhookMessage(webhookUrl, content);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // EMOJIS - Full CRUD
          // ============================================
          } else if (url.pathname === '/api/emojis/create' && req.method === 'POST') {
            // POST /api/emojis/create - Create an emoji
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, name, imageUrl } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!name || !imageUrl) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'name and imageUrl are required' }));
                  return;
                }
                const result = await discordService.createEmoji(guild, name, imageUrl);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/emojis/delete' && req.method === 'DELETE') {
            // DELETE /api/emojis/delete - Delete an emoji
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, emojiId } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!emojiId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'emojiId is required' }));
                  return;
                }
                const result = await discordService.deleteEmoji(guild, emojiId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // STICKERS - Full CRUD
          // ============================================
          } else if (url.pathname === '/api/stickers/create' && req.method === 'POST') {
            // POST /api/stickers/create - Create a sticker
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, name, description, tags, imageUrl } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!name || !description || !tags || !imageUrl) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'name, description, tags, and imageUrl are required' }));
                  return;
                }
                const result = await discordService.createSticker(guild, name, description, tags, imageUrl);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/stickers/delete' && req.method === 'DELETE') {
            // DELETE /api/stickers/delete - Delete a sticker
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, stickerId } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!stickerId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'stickerId is required' }));
                  return;
                }
                const result = await discordService.deleteSticker(guild, stickerId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // AUTOMOD - Full CRUD
          // ============================================
          } else if (url.pathname === '/api/automod/create' && req.method === 'POST') {
            // POST /api/automod/create - Create automod rule
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, name, triggerType } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!name) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'name is required' }));
                ...
                const result = await discordService.createAutomodRule(guild, name, triggerType || 'KEYWORD');
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/automod/edit' && req.method === 'PUT') {
            // PUT /api/automod/edit - Edit automod rule
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, ruleId, name } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!ruleId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'ruleId is required' }));
                  return;
                }
                const result = await discordService.editAutomodRule(guild, ruleId, name);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/automod/delete' && req.method === 'DELETE') {
            // DELETE /api/automod/delete - Delete automod rule
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, ruleId } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!ruleId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'ruleId is required' }));
                ...
                const result = await discordService.deleteAutomodRule(guild, ruleId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // ============================================
          // ROLES - Full CRUD (edit, delete already have create)
          // ============================================
          } else if (url.pathname === '/api/roles/edit' && req.method === 'PUT') {
            // PUT /api/roles/edit - Edit a role
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, roleId, name, color, permissions } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!roleId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'roleId is required' }));
                  return;
                }
                const result = await discordService.editRole(guild, roleId, name, color, permissions);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/roles/delete' && req.method === 'DELETE') {
            // DELETE /api/roles/delete - Delete a role
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, roleId } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!roleId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'roleId is required' }));
                ...
                const result = await discordService.deleteRole(guild, roleId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // ============================================
          // CHANNELS - Edit, Move, Permissions
          // ============================================
          } else if (url.pathname === '/api/channels/edit' && req.method === 'PUT') {
            // PUT /api/channels/edit - Edit a channel
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, name, topic, slowmode, isPrivate, categoryId } = JSON.parse(body);
                if (!channelId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId is required' }));
                  return;
                }
                const result = await discordService.editChannelAdvanced(undefined, channelId, { name, topic, slowmode, isPrivate, categoryId });
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/channels/move' && req.method === 'POST') {
            // POST /api/channels/move - Move channel to category
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, categoryId } = JSON.parse(body);
                if (!channelId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId is required' }));
                  return;
                }
                const result = await discordService.moveChannelToCategory(undefined, channelId, categoryId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/channels/permissions' && req.method === 'POST') {
            // POST /api/channels/permissions - Set channel permissions
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, targetId, targetType, permissions } = JSON.parse(body);
                if (!channelId || !targetId || !permissions) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId, targetId, and permissions are required' }));
                  return;
                }
                const result = await discordService.setChannelPermissions(channelId, targetId, targetType || 'role', permissions);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // MEMBERS - Edit member
          // ============================================
          } else if (url.pathname === '/api/members/edit' && req.method === 'PUT') {
            // PUT /api/members/edit - Edit a member
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, userId, nickname, mute, deaf } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!userId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'userId is required' }));
                ...
                const result = await discordService.editMember(guild, userId, nickname);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/members/timeout/remove' && req.method === 'POST') {
            // POST /api/members/timeout/remove - Remove timeout from member
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, userId } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!userId) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'userId is required' }));
                ...
                const result = await discordService.removeTimeout(guild, userId);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // EVENTS - Edit event
          // ============================================
          } else if (url.pathname === '/api/events/edit' && req.method === 'PUT') {
            // PUT /api/events/edit - Edit an event
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, eventId, name, startTime } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                if (!eventId || !name) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'eventId and name are required' }));
                  return;
                }
                const result = await discordService.editEvent(guild, eventId, name, startTime);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // EMBEDS & INTERACTIVE
          // ============================================
          } else if (url.pathname === '/api/embed' && req.method === 'POST') {
            // POST /api/embed - Send an embed message
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, title, description, color, fields, footer, thumbnail, image } = JSON.parse(body);
                if (!channelId || !title) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId and title are required' }));
                  return;
                }
                const result = await discordService.sendEmbed(channelId, title, description, color, fields, footer, thumbnail, image);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/button' && req.method === 'POST') {
            // POST /api/button - Send a button message
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, content, buttons } = JSON.parse(body);
                if (!channelId || !buttons || !Array.isArray(buttons)) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId and buttons array are required' }));
                  return;
                }
                const result = await discordService.sendButton(channelId, content || 'Click a button:', buttons);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/selectmenu' && req.method === 'POST') {
            // POST /api/selectmenu - Send a select menu
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { channelId, content, customId, placeholder, minValues, maxValues, options } = JSON.parse(body);
                if (!channelId || !options || !Array.isArray(options)) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'channelId and options array are required' }));
                  return;
                }
                const result = await discordService.sendSelectMenu(
                  channelId,
          ...
          // SERVER SETTINGS
          // ============================================
          } else if (url.pathname === '/api/server/edit' && req.method === 'PUT') {
            // PUT /api/server/edit - Edit server settings
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, ...options } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                const result = await discordService.editServer(guild, options);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/welcome/edit' && req.method === 'PUT') {
            // PUT /api/welcome/edit - Edit welcome screen
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                const { guildId, enabled, description } = JSON.parse(body);
                const guild = guildId || process.env.DISCORD_GUILD_ID;
                const result = await discordService.editWelcomeScreen(guild, enabled, description);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          // DMs
          // ============================================
          } else if (url.pathname === '/api/dm/send' && req.method === 'POST') {
            ...
            req.on('end', async () => {
              try {
                ...
                if (!userId || !message) {
                  res.writeHead(400, { 'Content-Type': 'application/json' });
                  res.end(JSON.stringify({ success: false, error: 'userId and message are required' }));
                  return;
                }
                const result = await discordService.sendPrivateMessage(userId, message);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, data: result }));
              } catch (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname === '/api/dm/read' && req.method === 'GET') {
            // GET /api/dm/read - Read DMs
            try {
              const userId = url.searchParams.get('userId');
              const count = url.searchParams.get('count') || '50';
              if (!userId) {
              ...
              const result = await discordService.readPrivateMessages(userId, count);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify({ success: true, data: result }));
            } catch (error) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
          ...
          } else if (url.pathname.startsWith('/api/tool/') && req.method === 'POST') {
            // POST /api/tool/:toolName - Execute a specific tool
            const toolName = url.pathname.replace('/api/tool/', '');
            let body = '';
            req.on('data', chunk => body += chunk.toString());
            req.on('end', async () => {
              try {
                ...
                // Use the same switch-case logic as JSON-RPC but exposed via REST
                // This allows the frontend to call any tool by name
                const message = {
                  method: 'tools/call',
                  params: { name: toolName, arguments: args }
                };

                // Emit a custom event or handle inline
                // For simplicity, we'll return an error asking to use JSON-RPC
                // In production, this would route to the tool handler
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({
                  success: true,
                  message: `Tool ${toolName} would be executed with args`,
                  tool: toolName,
                  args
                }));
              } catch (error) {
            ...
            res.end(JSON.stringify({
              ...
              endpoints: {
                mcp: {
                  sse: 'GET /sse - SSE connection for MCP protocol',
                  message: 'POST /message - Message handling for MCP',
                  jsonrpc: 'POST / - JSON-RPC over HTTP'
                },
                rest: {
                  // === READ ENDPOINTS ===
                  server: 'GET /api/server - Server info',
                  channels: 'GET /api/channels - List channels',
                  structure: 'GET /api/structure - Channel structure tree',
                  members: 'GET /api/members - List members',
                  member: 'GET /api/member?userId=xxx - Member details',
                  roles: 'GET /api/roles - List roles',
                  messages: 'GET /api/messages?channelId=xxx - Read messages',
                  pinned: 'GET /api/pinned?channelId=xxx - Pinned messages',
                  threads: 'GET /api/threads - Active threads',
                  events: 'GET /api/events - Scheduled events',
                  emojis: 'GET /api/emojis - Custom emojis',
                  stickers: 'GET /api/stickers - Custom stickers',
                  invites: 'GET /api/invites - Server invites',
                  webhooks: 'GET /api/webhooks?channelId=xxx - Channel webhooks',
                  automod: 'GET /api/automod - Automod rules',
                  bans: 'GET /api/bans - Server bans',
                  auditlog: 'GET /api/auditlog - Audit log',
                  voice: 'GET /api/voice - Voice connections',
                  widget: 'GET /api/widget - Server widget',
                  welcome: 'GET /api/welcome - Welcome screen',
                  permissions: 'GET /api/permissions?channelId=xxx - Channel permissions',
                  export: 'GET /api/export?channelId=xxx - Export chat log',
                  stats: 'GET /api/stats - Server statistics',
                  tools: 'GET /api/tools - List all MCP tools',
                  dmRead: 'GET /api/dm/read?userId=xxx - Read DMs',
                  // === MESSAGES CRUD ===
                  sendMessage: 'POST /api/messages - Send message',
                  editMessage: 'PUT /api/messages/edit - Edit message',
                  deleteMessage: 'DELETE /api/messages/delete - Delete message',
                  bulkDelete: 'POST /api/messages/bulk-delete - Bulk delete messages',
                  pinMessage: 'POST /api/messages/pin - Pin message',
                  unpinMessage: 'POST /api/messages/unpin - Unpin message',
                  addReaction: 'POST /api/messages/react - Add reaction',
                  removeReaction: 'POST /api/messages/unreact - Remove reaction',
                  // === THREADS CRUD ===
                  createThread: 'POST /api/threads/create - Create thread',
                  archiveThread: 'POST /api/threads/archive - Archive thread',
                  unarchiveThread: 'POST /api/threads/unarchive - Unarchive thread',
                  lockThread: 'POST /api/threads/lock - Lock thread',
                  unlockThread: 'POST /api/threads/unlock - Unlock thread',
                  // === WEBHOOKS CRUD ===
                  createWebhook: 'POST /api/webhooks/create - Create webhook',
                  deleteWebhook: 'DELETE /api/webhooks/delete - Delete webhook',
                  sendWebhook: 'POST /api/webhooks/send - Send via webhook',
                  // === EMOJIS CRUD ===
                  createEmoji: 'POST /api/emojis/create - Create emoji',
                  deleteEmoji: 'DELETE /api/emojis/delete - Delete emoji',
                  // === STICKERS CRUD ===
                  createSticker: 'POST /api/stickers/create - Create sticker',
                  deleteSticker: 'DELETE /api/stickers/delete - Delete sticker',
                  // === AUTOMOD CRUD ===
                  createAutomod: 'POST /api/automod/create - Create automod rule',
                  editAutomod: 'PUT /api/automod/edit - Edit automod rule',
                  deleteAutomod: 'DELETE /api/automod/delete - Delete automod rule',
                  // === ROLES CRUD ===
                  createRole: 'POST /api/roles/create - Create role',
                  editRole: 'PUT /api/roles/edit - Edit role',
                  deleteRole: 'DELETE /api/roles/delete - Delete role',
                  addRole: 'POST /api/roles/add - Add role to member',
                  removeRole: 'POST /api/roles/remove - Remove role from member',
                  // === CHANNELS CRUD ===
                  createChannel: 'POST /api/channels/create - Create channel',
                  editChannel: 'PUT /api/channels/edit - Edit channel',
                  deleteChannel: 'POST /api/channels/delete - Delete channel',
                  moveChannel: 'POST /api/channels/move - Move to category',
                  setPermissions: 'POST /api/channels/permissions - Set permissions',
                  // === MEMBERS ===
                  editMember: 'PUT /api/members/edit - Edit member',
                  removeTimeout: 'POST /api/members/timeout/remove - Remove timeout',
                  ban: 'POST /api/ban - Ban member',
                  unban: 'POST /api/unban - Unban member',
                  kick: 'POST /api/kick - Kick member',
                  timeout: 'POST /api/timeout - Timeout member',
                  // === EVENTS CRUD ===
                  createEvent: 'POST /api/events/create - Create event',
                  editEvent: 'PUT /api/events/edit - Edit event',
                  deleteEvent: 'POST /api/events/delete - Delete event',
                  // === INVITES CRUD ===
                  createInvite: 'POST /api/invites/create - Create invite',
                  deleteInvite: 'POST /api/invites/delete - Delete invite',
                  // === INTERACTIVE ===
                  sendEmbed: 'POST /api/embed - Send embed message',
                  sendButton: 'POST /api/button - Send button message',
                  sendSelectMenu: 'POST /api/selectmenu - Send select menu',
                  // === SERVER SETTINGS ===
                  editServer: 'PUT /api/server/edit - Edit server settings',
                  editWelcome: 'PUT /api/welcome/edit - Edit welcome screen',
                  // === DMs ===
                  sendDm: 'POST /api/dm/send - Send DM',
                  // === OTHER ===
                  executeTool: 'POST /api/tool/:toolName - Execute MCP tool'
                },
